...
/** <code>HLLCounter</code> allows for cardinality estimation of 
    ...
    <p>
    For guidance on setting precisions for a desired level of
    error, see the {@link com.adroll.cantor package info}.
*/
public class HLLCounter implements Serializable {
  ...
  public static final byte MIN_P = (byte)4;
  /** Maximum HLL precision of 18 */
  public static final byte MAX_P = (byte)18;
  ...
  */
  ...
  /**
     ...
     that can be used to estimate the cardinality of a set 
     of items with precision <code>DEFAULT_P</code>.
   */
  public HLLCounter() {
    set(DEFAULT_P, false, 0);
  }

  /**
     Constructs a non-intersectable <code>HLLCounter</code> object 
     that can be used to estimate the cardinality of a set of items 
     with given precision.

     @param p the <code>byte</code> precision of the HLL structure,
                  <code>MIN_P &lt;= p &lt;= MAX_P</code> 
   */
  public HLLCounter(byte p) {
    set(p, false, 0);
  }

  /**
     Constructs an <code>HLLCounter</code> that can be used to
     estimate the cardinality of a set of items with  
     <code>DEFAULT_P</code> and, if <code>intersectable</code> is 
     <code>true</code>, <code>DEFAULT_K</code>.

     @param intersectable <code>boolean</code> to make the structure 
                          intersectable
   */
  public HLLCounter(boolean intersectable) {
    set(DEFAULT_P, intersectable, DEFAULT_K);
  }

  /**
     Constructs an <code>HLLCounter</code> that can be used to 
     estimate the cardinality of a set of items with specified 
     precision and, if <code>intersectable</code> is 
     <code>true</code>, <code>k</code> is a reasonable precision
     guess based on <code>p</code>.

     @param p             the <code>byte</code> precision of the 
                          HLL structure, <code>MIN_P &lt;= p &lt;= 
                          MAX_P</code>
     @param intersectable <code>boolean</code> to make the structure 
                          intersectable
   */
  public HLLCounter(byte p, boolean intersectable) {
    // Convenient MinHash precision estimation, based on HLL precision.
    set(p, intersectable, (int)Math.max(Math.pow(2, p - 5), 128));
  }
  ...
  /**
     ...
     estimate the cardinality of a set of items with 
     <code>DEFAULT_P</code> and, if <code>intersectable</code> is 
     <code>true</code>, specified <code>k</code>.
     ...
     @param k             the <code>int</code> precision of MinHash 
                          structure
   */
  public HLLCounter(boolean intersectable, int k) {
    set(DEFAULT_P, intersectable, k);
  }
  ...
  /**
     ...
     @param k             the <code>int</code> precision of MinHash 
                          structure
   */
  public HLLCounter(byte p, boolean intersectable, int k) {
    set(p, intersectable, k);
  }
  ...
  /**
     ...
     estimate the cardinality of a set of items with specified 
     precision and, if <code>intersectable</code> is 
     <code>true</code>, specified <code>k</code>, along with 
     pre-computed HLL and MinHash structures.
     <p>
     One should only use this constructor when absolutely 
     necessary, for example to copy one <code>HLLCounter</code>'s 
     fields into a new <code>HLLCounter</code>.
  ...
  /**
     Helper method to perform basic sanity checks on constructor inputs.
     ...
     @param k             the <code>int</code> precision of MinHash 
                          structure
   */
  private void set(byte p, boolean intersectable, int k) {
    if(p < MIN_P) {
      throw new IllegalArgumentException("HLLCounter precisions cannot be less than MIN_P = " +
                                         Byte.toString(MIN_P));
    }
    if(p > MAX_P) {
      throw new IllegalArgumentException("HLLCounter precisions cannot be greater than MAX_P = " +
                                         Byte.toString(MAX_P));
    }
  ...
  /**
     ...
     <code>TreeSet&lt;Long&gt;</code> if it is one of the <code>k</code> 
  ...
  /**
     Insert multiple elements into the <code>HLLCounter</code> 
     structure.
     <p>
     This iterates the {@link #put(String v)} method.

     @param vs the <code>String...</code> elements to insert
  */
  public void put(String ... vs) {
    for(String v : vs) {
      this.put(v);
    }
  }
  ...
  /**
     Returns the estimated number of unique insertions into
     the <code>HLLCounter</code> structure.
     <p>
     For sets of cardinality less than <code>k</code>, this
     is exact.

     @return the <code>long</code> estimated size of unique
             insertions
  */
  public long size() {
    // If we haven't seen k elements yet, the size is exactly the size of ts
  ...
  */
  ...
  /**
     Performs a destructive union of this 
     <code>HLLCounter</code> and the one passed in.
     <p>
     This <code>HLLCounter</code> will now contain data that is 
     equivalent to all insertions from both 
     <code>HLLCounter</code>s going into a single 
     <code>HLLCounter</code>. The precisions of this 
     <code>HLLCounter</code> will be the smaller of the precisions 
     between each <code>HLLCounter</code>.
  ...
  /**
     Performs a destructive union of this <code>HLLCounter</code>
     and all the ones passed in.
     <p>
     This iterates the {@link #combine(HLLCounter h)} method.

     @param hs the <code>HLLCounter...</code> to combine into 
               this one
  */
  ...
  */
  ...
  /**
     ...
     @return the <code>byte</code> precision of the HLL
  */
  public byte getP() {
    return p;
  }
  ...
  /**
     ...
     @return the <code>boolean</code> that indicates
             whether this is intersectable
  */
  ...
  /**
     ...
     @return the <code>TreeSet&lt;Long&gt;</code> of the
             <code>k</code> least elements hashed
  */
  ...
  /**
     ...
     @return the <code>int</code> precision of MinHash
  */
  public int getK() {
    return k;
  }
  ...
  /**
    ...
    of the given <code>HLLCounters</code>.
    <p>
    This method only works for intersectable 
    <code>HLLCounters</code>. It doesn't actually perform 
    an intersection, but rather returns an estimate of the 
    cardinality of the intersection.
    <p>
    It calculates a global MinHash structure based on the
    minimum MinHash precision <code>k</code> across all the 
    structures. A global HLL structure is also computed
    with the minimum precision across all structures. That
    is, the estimate's only as accurate as the least accurate
    <code>HLLCounter</code>.

    @param hs the <code>HLLCounter...</code> to "intersect"

    @return   the <code>long</code> estimate of the intersection
              cardinality
  */
  public static long intersect(HLLCounter ... hs) {
  ...
  */
  ...
  /**
     Returns the HLL structure with reduced precision <code>q</code>.
     <p>
  ...
  */
  ...
  /**
     Returns an estimate of the size of the union of a series of
     <code>HLLCounter</code>s non-destructively.
     <p>
     This is a nondestructive form of 
     {@link #combine(HLLCounter ... hs)}.

     @param hs the <code>HLLCounter...</code> to union
  ...
  */
  ...
  /**
     ...
     @param m the <code>int</code> length of the HLL structure
  ...
  */
  ...
  /**
     Returns the approximate number of unique insertions into
     an HLL structure.
     <p>
     For low cardinalities, the LINEARCOUNTING method is used.
     From there, a bias estimate is used to correct for higher
     cardinalities, but cardinalities that are still not within
  ...
  */
  ...
  /**
     ...
     @param E the <code>double</code> estimate of cardinality
     @param w the <code>byte</code> precision of HLL structure 
              used for estimate

     @return  the <code>double</code> bias estimate
  */
  private static double estimateBias(double E, byte w) {
    int idx = (int)(w - MIN_P);
    double[] red = rawEstimateData[idx];
    double[] bd = biasData[idx];
    if(E <= red[0]) {
      return bd[0];
    }
    if(E > red[red.length - 1]) {
      return 0.0;
    }
    for(int i = 0; i < red.length - 2; i++) {
      if(red[i] < E && E < red[i + 1]) {
        double slope = (bd[i + 1] - bd[i])/(red[i + 1] - red[i]);
        double intercept = bd[i + 1] - (slope * red[i + 1]);
  ...
  /**
     Returns the <code>long</code> that is the result 
     of a hash.

     @param x the <code>byte[]</code> of the first eight
              bytes of the hash
  ...
  */
  ...
  private static double[] powers = {
    ...
    0.001953125, 9.765625E-4, 4.8828125E-4, 2.44140625E-4, 1.220703125E-4,
    6.103515625E-5, 3.0517578125E-5, 1.52587890625E-5, 7.62939453125E-6,
    3.814697265625E-6, 1.9073486328125E-6, 9.5367431640625E-7,
    4.76837158203125E-7, 2.384185791015625E-7, 1.1920928955078125E-7,
    5.9604644775390625E-8, 2.9802322387695312E-8, 1.4901161193847656E-8,
    7.450580596923828E-9, 3.725290298461914E-9, 1.862645149230957E-9,
    9.313225746154785E-10, 4.6566128730773926E-10, 2.3283064365386963E-10,
    ...
    2.8421709430404007E-14, 1.4210854715202004E-14, 7.105427357601002E-15,
    ...
    8.673617379884035E-19, 4.3368086899420177E-19, 2.1684043449710089E-19,
    ...
    1.6940658945086007E-21, 8.470329472543003E-22, 4.2351647362715017E-22,
    2.1175823681357508E-22, 1.0587911840678754E-22, 5.293955920339377E-23,
    2.6469779601696886E-23, 1.3234889800848443E-23, 6.6174449004242214E-24,
    3.3087224502121107E-24, 1.6543612251060553E-24, 8.271806125530277E-25,
    ...
    1.0097419586828951E-28, 5.0487097934144756E-29, 2.5243548967072378E-29,
    ...
    3.8518598887744717E-34, 1.9259299443872359E-34, 9.629649721936179E-35,
    4.8148248609680896E-35, 2.4074124304840448E-35, 1.2037062152420224E-35,
    ...
    1.8367099231598242E-40, 9.183549615799121E-41, 4.5917748078995606E-41,
    2.2958874039497803E-41, 1.1479437019748901E-41, 5.739718509874451E-42,
    2.8698592549372254E-42, 1.4349296274686127E-42, 7.1746481373430634E-43,
    3.5873240686715317E-43, 1.7936620343357659E-43, 8.968310171678829E-44,
    ...
    7.006492321624085E-46, 3.5032461608120427E-46, 1.7516230804060213E-46,
    ...
    4.1761948595190557E-53, 2.0880974297595278E-53, 1.0440487148797639E-53,
    5.2202435743988196E-54, 2.6101217871994098E-54, 1.3050608935997049E-54,
    ...
    1.5930919111324523E-58, 7.965459555662261E-59, 3.9827297778311307E-59,
    ...
    1.8546030753437107E-68, 9.273015376718553E-69, 4.6365076883592767E-69,
    ...
    8.843436600416711E-75, 4.4217183002083556E-75, 2.2108591501041778E-75,
    1.1054295750520889E-75, 5.5271478752604446E-76, 2.7635739376302223E-76,
  ...
  /**
     ...
     <p>
     These data come from <a href="http://goo.gl/iU8Ig">this addendum</a>.
  */
  private static double[][] rawEstimateData = {
    // precision 4
  ...
  /**
     ...
     and corresponding to {@link #rawEstimateData}; by interpolation,
     it is possible to get other bias estimates based on these empirical data.
     <p>
     These data come from <a href="http://goo.gl/iU8Ig">this addendum</a>.
  */
  private static double[][] biasData = {
    // precision 4
